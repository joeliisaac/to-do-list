...
function createItem(item) {
  ...
  // TODO: Chanwge the id to "item_" + id
new_item.prop("id", 'item_' + id);
...
new_item.find('#item_checkbox_tmpl').val(id);
  // TODO: Change the id of the checkbox to "item_checkbox_" + id
new_item.find('#item_checkbox_tmpl').prop("id", "item_checkbox_" + id);
...
new_item.find(`#item_checkbox_${id}`).on('change', itemCheckboxListener);
  // return the new ToDo item's html that was just prepared.
  console.log(typeof(new_item),new_item);
  return new_item;
}
   ...
   */
   ...
   $.ajax({
       ...
       success: function(response){
         ...
         if(is_complete){
           $(`#item_${item_id}`).addClass("completed");
         }
         else{
           $(`#item_${item_id}`).removeClass("completed");
         }
       }
   });
...
data_text = {text: $("#item_input").val()};
   $.ajax({
   dataType: "json",
   url: "engine.php?a=add",
   method: "post",
   data: data_text,
   success: function(data){

    $("#item_input").val("");
    let new_item = createItem(data.item);
